...
void ZigbeeAnalogSensor::setAnalog(float analog) {
  float zb_analog = analog / 1000000.0f;
  log_v("Updating sensor value...");
  /* Update sensor measured value */
  log_d("Setting value to %0.1f", analog);
  esp_zb_lock_acquire(portMAX_DELAY);
  esp_zb_zcl_set_attribute_val(
    _endpoint, ESP_ZB_ZCL_CLUSTER_ID_ANALOG_OUTPUT, ESP_ZB_ZCL_CLUSTER_SERVER_ROLE, ESP_ZB_ZCL_ATTR_ANALOG_OUTPUT_PRESENT_VALUE_ID,
    &zb_analog, false
  );
  esp_zb_lock_release();
}
